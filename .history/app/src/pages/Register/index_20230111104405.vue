<template>
  <div class="register-container">
    ...
    <div class="register">
      ...
      <div class="content">
        <label>确认密码:</label>
        <input type="password" placeholder="请输入确认密码" v-model="psdagain" name="psdagain" v-validate="{ required:true,is:password }" :class="{invalid:errors.has('psdagain')}">
        <span class="error-msg">{{ errors.first("psdagain") }}</span>
      </div>
      <div class="controls">
        <input name="m1" type="checkbox" :checked="agree" name="psdagain" v-validate="{ required:true,is:password }" :class="{invalid:errors.has('psdagain')}">
...
<script>
  export default {
    ...
    methods: {
      ...
      async userRegister(){
        try {
          const {phone,code,password,psdagain} = this;
          (phone&&code&&password==psdagain) && await this.$store.dispatch('userRegister',{phone,code,password});
          // 如果成功，路由跳转到登录界面
