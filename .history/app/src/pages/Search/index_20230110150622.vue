...
export default {
  ...
  computed:{
    // mapGetters 里面的写法：传递的数组，因为getters计算是没有划分模块的，可以直接使用
  ...
  methods: {
    ...
    // 删除分类的名字
    removeCaregoryName(){
      // 把带给服务器的参数置空了，还需要问服务器发请求
      // 带给服务器参数说明可有可无：如果属性值为空的字符串还是会把响应的字段带给服务器
      ...
      this.getDate();
       // 地址栏也需要改，进行路由跳转,但是如果带有params参数，需要保留params参数
       // 判断：如果有params参数，将params参数保留，如果没有就不展示
      if(this.$route.query){
    ...
    removekeyword(){
      ...
      // 通知兄弟组件Header清除关键字
      this.$bus.$emit("clear");
      // 地址栏也需要改，进行路由跳转,但是如果带有query参数，需要保留query参数
       // 判断：如果有query参数，将query参数保留
      if(this.$route.query){
    ...
    // 排序的操作
    changeOrder(flag){
      // flag形参：它是一个标记，代表用户点击的是综合还是价格
      // 这里获取到的是最开始的状态
      let originFlag = this.searchParams.order.split(":")[0];
      let originSort = this.searchParams.order.split(":")[1];
      // 准备一个新的order属性值
      let newOrder = "";
      // 判断点击的是综合还是价格，如果还是点击的本身，则本身进行变换
      if(flag == originFlag){
  ...
  watch:{
    ...
    $route(){
      ...
      // 再次发起ajax请求
      this.getDate();
      // 每一次请求完毕，应该把相应的1、2、3级分类的id置为空，让他接受下一次相应的1、2、3级分类的id
