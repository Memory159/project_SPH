  ...
  <div class="cart">
      ...
      <div class="cart-body">
        <ul class="cart-list" v-for="(cart) in cartInfoList" :key="cart.id"> 
          ...
          <li class="cart-list-con5">
            <a href="javascript:void(0)" class="mins" @click="handler('minus',-1,cart)" :value="(cart.skuNum=cart.skuNum<=1?1:cart.skuNum)">-</a>
    ...
    <div class="cart-tool">
      <div class="select-all">
        <input class="chooseAll" type="checkbox" :checked="isAllCheck" @click="updateChecked(cart,$event)">
        <span>全选</span>
      </div>
      <div class="option">
  ...
  export default {
    ...
    methods: {
      ...
      async handler(type,disNum,cart){
        ...
        switch(type){
          case 'add':
            ...
            break;
          case 'minus':
            // 判断如果产品的个数大于1，才能传递给服务器-1
            // 如果出现产品的个数小于等于1，传递给服务器个数0（原封不动）
            disNum = (cart.skuNum > 1) ? -1 : 0;
            break;
          case 'change':
            // 用户输入进来的是最终量，如果非法的（带有汉字|出现负数），带给服务器数字零
            if(isNaN(disNum) || disNum < 1){
              disNum = 0
            }else{
              // 正常情况下，对这个数取整，带给服务器变化的量 = 用户输进来的 - 产品的起始个数
      ...
      updateChecked(cart,event){
        try {
          // 判断产品的勾选状态，其中checked返回的是布尔值，需要转换为0|1
          let isChecked = event.checked ? 0 : 1;
    ...
    computed:{ 
      ...
      isAllCheck(){
        // 采用数组的 every 方法
        // 用法：只有全部元素的isChecked属性都为真，结果才为真，只要有一个为假，结果就为假
        return this.cartInfoList.every(item=>item.isChecked == 1);
        // 也可以用for循环遍历，采用every更加简便
