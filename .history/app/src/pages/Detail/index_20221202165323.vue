...
export default {
  ...
  methods: {
    ...
    // 表单元素修改产品个数
    changeSkuNum(event){
      // 用户输入进来的文本 * 1，因为文本如果不是纯数字的话，乘以1以后就会变为NaN
      let value = event.target.value * 1;
      // 如果用户输入进来的数据非法，出现NaN或者小于1
      if(isNaN(value) || value < 1){
        this.skuNum = 1;
      }else{
        // 用户输入正常，但是数据为小数，对数据进行取整
        this.skuNum = parseInt(value);
      }
    },
    // 加入购物车
    async addShopcar(){
      // 1、再点击加入购物车这个按钮的时候，做的第一件事请，就是将参数带给服务器（发请求），通知服务器加入购物车的产品是谁
      // 2、你需要知道这次请求成功还是失败，如果成功进行路由跳转，如果失败，需要给用户提示
      try {
        // 成功
        await this.$store.dispatch('addOrUpdateShopCart',{skuId:this.$route.params.skuid,skuNum:this.skuNum});
        // 3、进行路由跳转
        // 4、在路由跳转的时候还需要将产品的信息传递过去
